...
bool Adafruit_BNO055::begin(adafruit_bno055_opmode_t mode)
{
  ...
  if(id != BNO055_ID)
  {
    gpioSleep(PI_TIME_RELATIVE, 1, 0); // hold on for boot
  ...
  {
    gpioSleep(PI_TIME_RELATIVE, 0, 1000*40);
  }
...
std::cout << "Reset finished" << std::endl;
  ...
  */
  ...
  write8(BNO055_AXIS_MAP_CONFIG_ADDR, REMAP_CONFIG_P4); // P0-P7, Default is P1
  gpioSleep(PI_TIME_RELATIVE, 0, 1000*10);
...
void Adafruit_BNO055::setMode(adafruit_bno055_opmode_t mode)
{
  int Count = 0;
  _mode = mode;
  while(!write8(BNO055_OPR_MODE_ADDR, _mode) && Count < 10)
  {
	  Count++;
  }
...
imu::Quaternion Adafruit_BNO055::getQuat(void)
{
  ...
  bool ReadB = readLen(BNO055_QUATERNION_DATA_W_LSB_ADDR, buffer, 8); 
  while(!ReadB && (count < 5)) 
  {
	count++;
	gpioSleep(PI_TIME_RELATIVE, 0, 300);
    ReadB = readLen(BNO055_QUATERNION_DATA_W_LSB_ADDR, buffer, 8); 
  }
...
void Adafruit_BNO055::setSensorOffsets(const adafruit_bno055_offsets_t &offsets_type)
{
    ...
    while(!write8(ACCEL_OFFSET_X_LSB_ADDR, (offsets_type.accel_offset_x) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(ACCEL_OFFSET_X_MSB_ADDR, (offsets_type.accel_offset_x >> 8) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(ACCEL_OFFSET_Y_LSB_ADDR, (offsets_type.accel_offset_y) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(ACCEL_OFFSET_Y_MSB_ADDR, (offsets_type.accel_offset_y >> 8) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(ACCEL_OFFSET_Z_LSB_ADDR, (offsets_type.accel_offset_z) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(ACCEL_OFFSET_Z_MSB_ADDR, (offsets_type.accel_offset_z >> 8) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(GYRO_OFFSET_X_LSB_ADDR, (offsets_type.gyro_offset_x) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(GYRO_OFFSET_X_MSB_ADDR, (offsets_type.gyro_offset_x >> 8) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(GYRO_OFFSET_Y_LSB_ADDR, (offsets_type.gyro_offset_y) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(GYRO_OFFSET_Y_MSB_ADDR, (offsets_type.gyro_offset_y >> 8) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(GYRO_OFFSET_Z_LSB_ADDR, (offsets_type.gyro_offset_z) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(GYRO_OFFSET_Z_MSB_ADDR, (offsets_type.gyro_offset_z >> 8) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(MAG_OFFSET_X_LSB_ADDR, (offsets_type.mag_offset_x) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(MAG_OFFSET_X_MSB_ADDR, (offsets_type.mag_offset_x >> 8) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(MAG_OFFSET_Y_LSB_ADDR, (offsets_type.mag_offset_y) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(MAG_OFFSET_Y_MSB_ADDR, (offsets_type.mag_offset_y >> 8) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(MAG_OFFSET_Z_LSB_ADDR, (offsets_type.mag_offset_z) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(MAG_OFFSET_Z_MSB_ADDR, (offsets_type.mag_offset_z >> 8) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
	gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(ACCEL_RADIUS_LSB_ADDR, (offsets_type.accel_radius) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(ACCEL_RADIUS_MSB_ADDR, (offsets_type.accel_radius >> 8) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
	gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(MAG_RADIUS_LSB_ADDR, (offsets_type.mag_radius) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
    gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    while(!write8(MAG_RADIUS_MSB_ADDR, (offsets_type.mag_radius >> 8) & 0x0FF))
    {
		gpioSleep(PI_TIME_RELATIVE, 0, 100000);
    }
	gpioSleep(PI_TIME_RELATIVE, 0, 100000);
...
bool Adafruit_BNO055::write8(adafruit_bno055_reg_t reg, uint8_t value)
{
  ...
  char FrameW[5] = {0xAA, 0x00, reg, 1, value};
  
  int Wout = serWrite (_HandleBNO, FrameW, 5);
  while ((serDataAvailable(_HandleBNO) < 2) && CountWD < 20)
  {
	gpioSleep(PI_TIME_RELATIVE, 0, 500);
	CountWD++;
  }
	  
  serRead(_HandleBNO, ReadBuff, 2);
  
  if ((Wout != 0) || (ReadBuff[1] != 0x01))
  {
	//std::cout << (int) CountWD <<"Ewrite: " << (int) ReadBuff[1] << std::endl;
	CountWD = 0;
	int DAvail = serDataAvailable(_HandleBNO);
	if (DAvail > 0)
	{
		serRead(_HandleBNO, ReadBuff, DAvail);
		memset(ReadBuff, 0, sizeof(DAvail));
	}
	gpioSleep(PI_TIME_RELATIVE, 0, 15000);
	serWrite (_HandleBNO, FrameW, 5);
	while ((serDataAvailable(_HandleBNO) < 2) && CountWD < 20)
	{
		gpioSleep(PI_TIME_RELATIVE, 0, 500);
		CountWD++;
	...
	{
		//std::cout << (int) CountWD <<"Error write: " << (int) ReadBuff[1]<< std::endl;
		return false;
	}
...
uint8_t Adafruit_BNO055::read8(adafruit_bno055_reg_t reg )
{
  ...
  uint8_t value[3] = {0x00, 0x00, 0x00}, CountWD = 0;
  char FrameW[4] = {0xAA, 0x01, reg, 1};
  
  serWrite (_HandleBNO, FrameW, 4);
  while ((serDataAvailable(_HandleBNO) < 3) && CountWD < 20)
  {
	gpioSleep(PI_TIME_RELATIVE, 0, 500);
	CountWD++;
  }
  int BRead = serRead(_HandleBNO, (char*)value, 3);
   
  if ((BRead != (int) 3) || (value[0] != 0xBB))
  {
	//std::cout <<"Error read: " << (int) value[1] <<  std::endl;
	return -1;
  }
...
bool Adafruit_BNO055::readLen(adafruit_bno055_reg_t reg, uint8_t * buffer, uint8_t len)
{
  ...
  char FrameW[4] = {0xAA, 0x01, reg, len};
  
  if (serWrite (_HandleBNO, FrameW, 4))
	  return false;
 
  while ((serDataAvailable(_HandleBNO) < (len+2)) && CountWD < 20)
  {
	gpioSleep(PI_TIME_RELATIVE, 0, 500);
	CountWD++;
  }

  int BRead = serRead(_HandleBNO, ReadBuff, len+2);

  if ((BRead != (int) len+2) || (ReadBuff[0] != 0xBB)) 
	return false;

  memcpy(buffer, ReadBuff + 2, len*sizeof(uint8_t));
  return true;
}
